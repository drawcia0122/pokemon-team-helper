...
前月分を毎週確認する自動更新Workflowがあります。既存の取得・正規化処理の後に
Validationと前回Dataset比較を行い、cutoff 0/1760の両方が正常な場合だけ公開します。
失敗時は直前の正常Datasetを維持し、同一content hashではcommit・Deployしません。
...
# 自動更新、異常検知、Fallback、Single Source of Truthのテスト
npm run test:environment-pipeline

# 前月分を本番JSONへ書き込まず確認
npm run environment:update -- --dry-run

# 保存済みDatasetの構造とMetadataを確認
...
自動更新は`.github/workflows/refresh-environment-data.yml`で毎週火曜06:23 UTCに実行します。
手動実行は`dry_run: true`が初期値です。Dataset Metadataは取得元、取得日時、公開日時、
regulation、対象月、cutoff、使用率の内部単位、checksum、件数を保持します。
取得元の使用率が`12.5%`、`12.5`、`0.125`のいずれでも内部値`0.125`へ統一します。
使用率・採用率は0〜1で、Advisorの最低使用率は`0.001`です。

Validationでは必須項目、型、数値範囲、TOP10、技・特性、canonical ID、フォーム重複、
regulation・対象月・cutoff、未知名称を確認します。Compareではポケモン件数25%超減、
TOP10の80%超入れ替え、技・特性50%超減、使用率合計30%超変化、
regulation変更、対象月巻き戻りを異常としてPublishしません。

FetchはHTTPS allowlist、15秒timeout、20 MiB上限、最大3回のexponential backoffを
使用します。統計データ固有の明示ライセンスは確認できないため、Smogonの公開JSONだけを
低頻度で取得し、出典を表示します。運用・dry-run・Publish・Fallbackの詳細は
