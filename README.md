...
画面では従来のSTEP 4と統合し、「現在の課題」「改善候補」「詳細診断」を縦に配置します。
詳細診断には、従来の補完タイプ、補完スコア候補、追加前後比較、4位以下ランキングを残し、
Advisorの改善候補と同じspeciesは重複表示しません。総合評価は正式な算出方法を定めるまで
表示しません。

初版では技の効果、特性の効果、持ち物、テラスタイプ、実ダメージ、交代先としての確定性を
判定できません。表示は追加・入れ替えを検討するための参考提案であり、勝敗や安全な受けを
保証するものではありません。将来の改善量、おすすめ入れ替え、天候・速度操作・最適化は
AdvisorEngineの入力と評価signalを拡張して追加します。
