...
assert(siteNavigationItems.length === 3, "ナビゲーションリンクが3件ではありません");
assert(
  JSON.stringify(siteNavigationItems.map((item) => item.href)) === JSON.stringify(["/", "/builds", "/news"]),
  "ナビゲーションのリンク先が不正です"
);

for (const active of ["team", "builds", "news"]) {
